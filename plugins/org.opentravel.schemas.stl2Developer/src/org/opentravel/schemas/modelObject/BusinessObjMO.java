...
public class BusinessObjMO extends ModelObject<TLBusinessObject> {
    ...
    private void firePropertyChange(Events propertyName, Object oldValue, Object newValue) {
        for (PropertyChangeListener l : listeners) {
            l.propertyChange(new PropertyChangeEvent(this, propertyName.toString(), oldValue,
                    newValue));
        }
    }
        ...
        if (getTLBase() != null) {
            if (inheritedFacetListener == null) {
                throw new IllegalStateException(
                        "Extended BO has to listen on changes on base type.");
            }
            DefaultModelController modelC = (DefaultModelController) OtmRegistry
                    .getMainController().getModelController();
    ...
    public void updateFacet(TLFacet affectedItem) {
        TLFacet exist = findFacet(affectedItem.getLabel(), affectedItem.getContext(),
                affectedItem.getFacetType());
        if (exist == null) {
    ...
    protected void removeInheritedFacet(TLFacet affectedItem) {
        if (TLFacetType.CUSTOM.equals(affectedItem.getFacetType())) {
            TLFacet removedFacet = getTLModelObj().getCustomFacet(affectedItem.getContext(),
                    affectedItem.getLabel());
            ...
            TLFacet removedFacet = getTLModelObj().getQueryFacet(affectedItem.getContext(),
                    affectedItem.getLabel());
            if (removedFacet != null && !isNotEmpty(removedFacet)) {
    ...
    public void addCustomFacet(final TLFacet tlf) {
        ...
        LOGGER.info("Added custom facet " + tlf.getLocalName() + " to BusinessObject "
                + this.getName());
    }

    public void addQueryFacet(final TLFacet tlf) {
        srcObj.addQueryFacet(tlf);
        LOGGER.info("Added query facet " + tlf.getLocalName() + " to BusinessObject "
                + this.getName());
    }
    ...
    private boolean isNotEmpty(TLFacet facet) {
        ...
        ret = ret
                || (facet.getDocumentation() != null ? !facet.getDocumentation().isEmpty() : false);
