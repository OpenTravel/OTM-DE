...
public class RestResourceView extends OtmAbstractView implements ISelectionListener, ISelectionChangedListener,
	...
	public void createPartControl(final Composite parent) {
		...
		rName = toolkit.createText(compLeft, "", SWT.BORDER);
		rName.setToolTipText(Messages.getString("context.label.applicationContext.tooltip"));
		rName.setLayoutData(textGD);
		rName.addModifyListener(new ModifyListener() {
		...
		rDescriptionText = toolkit.createText(compLeft, "", SWT.BORDER);
		rDescriptionText.setLayoutData(textGD);

		rDescriptionText.addModifyListener(new ModifyListener() {
		...
		// cloneContextAction = new CloneContextAction(mainWindow, new
		// ExternalizedStringProperties("action.cloneContext"));
		// mergeContextAction = new MergeContextAction(mainWindow, new
		// ExternalizedStringProperties("action.mergeContext"));
	...
	private void updateFields(ContextNode context) {
		if (context != null) {
			rName.setData(context);
			// ctxIdText.setData(context);
			rDescriptionText.setData(context);
			rName.setText(context.getApplicationContext());
			// ctxIdText.setText(context.getContextId());
			rDescriptionText.setText(context.getDescription());
			retrieveDefaultContext(context);
		} else {
			rName.setData(null);
			// ctxIdText.setData(null);
			rDescriptionText.setData(null);
			rName.setText("");
			// ctxIdText.setText("");
			rDescriptionText.setText("");
		}
		boolean enabled = false;
		if (context != null && context.getLibraryNode().isEditable())
			enabled = true;
		rName.setEnabled(enabled);
		// ctxIdText.setEnabled(enabled);
		rDescriptionText.setEnabled(enabled);
	}
	...
	public String getDescriptionText() {
		return rDescriptionText.getText();
	}
