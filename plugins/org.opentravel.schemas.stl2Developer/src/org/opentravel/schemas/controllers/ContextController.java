...
/**
 * Controller for the OTM Context identifiers. Context is defined in the model as an application context (URI) and ID.
 * IDs are for the user to select which context to apply/view.
 * 
 * Implements the Model View Controller Design Pattern. Controller handle all requests coming from the view, user
 * interface or application. The controller is responsible for accessing model.
 * 
 * Unlike some MVC implementations, this implementation is designed to have a single controller for the whole
 * application and not individual controllers for each view.
 * 
 * Controller view contexts exposed in the functions are: Default - Each library in a model may have multiple contexts,
 * one of which is designated the "default" context. Selected - the context currently selected in the context view tree.
 * 
...
public interface ContextController {
	...
	// /**
	// * Clears the contexts from the model. Clears the selected contexts. Invoke when the model is created or changed
	// */
	// void clearContexts();
	...
	/**
	 * @return the selected context id for the specified view. If the selected node is in a different library, then the
	 *         default for that library is returned and set as selected.
	 */
	...
	/**
	 * Create a new context to the context list of the currently selected context node. Get the library from the context
	 * view or navigation selected. Use ID and application values from the context view.
	 */
	void newContext();
