...
public class OtmHandlers {
     ...
     */
    public void enableDropTarget(final Control widget, final OtmActions actionContext,
            final int action, final OtmWidgets initContext) {
    ...
    /**
     * Drag-n-drop onto a property type field. As a drag-n-drop, there is no widget associated with
     * the event. The nodeID is used and will be passed to the operation. DropTargetListener - stop
     * any drop from happening on the current target by setting the event.detail field to
     * DND_DROP_NONE.
     */
    public final class PropertyDropTarget extends DropTargetAdapter {
        ...
        /**
         * dropAccept is called just before the drop is performed. The drop target is given the
         * chance to change the nature of the drop or veto the drop by setting the event.detail
         * field Note - event data is null! Can not find the dragged object (source) from the event
        ...
        public void dropAccept(final DropTargetEvent event) {
            ...
            if (event.item instanceof TableItem) {
                ...
                if ((ti != null) && (ti.getData() != null)
                        && ((ti.getData() instanceof ComponentNode))) {
                    ...
                    if (dragSourceNode != null) {
                        ...
                        if (!tNode.isEditable()) {
                            ...
                            DialogUserNotifier.openInformation("WARNING",
                                    "Object is not editable, drop was ignored.");
                            event.detail = DND.DROP_NONE;
                        }
        ...
        /**
         * dragEnter is called when the cursor has entered the drop target boundaries. For facet
         * table, it is the whole table not just items.
         */
        @Override
    ...
    /**
     * Drag listener for the tree view see:
    ...
    private final class OtmTreeDragSourceListener implements DragSourceListener {
        ...
        /**
         * Loads a dnd event with data. Called only AFTER the drop target has accepted the event but
         * before the drop.
         */
        @Override
    ...
    /**
     * If dirty, do the event. Can't wait for focus event because a CR may not cause focus to
     * change.
     */
    ...
    /**
     * if there have been changes to the widget, call its action handler. Focus is controlled to
     * prevent TextModify events when the widget has text loaded in from being processed.
     * 
     * @author Dave Hollander
     * 
     */
        ...
        public void focusLost(final FocusEvent e) {
            ...
            if (o instanceof OtmEventData) {
                ...
                if (e.widget instanceof Text) {
                    ...
                    wd.setText((t.getText() == null) ? "" : stripInvalidXMLCharacters(t.getText()
                            .trim()));
     ...
     * 
     * Fires when data is set into the widget and when characters are typed. Question - do i want to
     * make changes now or on focus loss? if i do it now, all the setters will need to return
     * change/no-change state doing it while typing is distracting and causes the cursor to move.
     * Wait until done then do the event.
     * 
        ...
        public void modifyText(final ModifyEvent e) {
            ...
            if (o instanceof OtmEventData) {
                ...
                if (e.widget instanceof Text) {
                    ...
                    eventData.setText((t.getText() == null) ? "" : stripInvalidXMLCharacters(t
                            .getText().trim()));
