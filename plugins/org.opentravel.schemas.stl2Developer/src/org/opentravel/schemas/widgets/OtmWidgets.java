...
public class OtmWidgets {
	 ...
	 *            Composite
	 * @param properties
	 *            - one of the definitions from the class containing indexes into messages.properties
	 * @param event
	...
	public void assignButtonEvent(final Button button, final Node n, final int event, final String label,
			final String toolTip, final SelectionAdapter handler) {
	...
	public Spinner formatSpinner(final Composite parent, final int[] properties, final int event, final SpinnerData sd,
			final SelectionAdapter handler) {
	...
	/**
	 * Post a sash in the current grid. The 1st field does not get a visible sash, width-1 fields do.
	 * 
	 * @param parent
	 * @param width
	 * @param color
	...
	public Combo formatCombo(final FormToolkit toolkit, final Composite parent, final int[] properties,
	 ...
	 */
	public Combo formatCombo(final Composite parent, final int[] properties, final String[] values, final int event,
			final ModifyListener handler, final boolean editable) {
		final OtmEventData wd = new OtmEventData();
		wd.label = Messages.getString("OtmW." + properties[0]); //$NON-NLS-1$
		final String toolTip = Messages.getString("OtmW." + properties[1]); //$NON-NLS-1$
		wd.businessEvent = event;
