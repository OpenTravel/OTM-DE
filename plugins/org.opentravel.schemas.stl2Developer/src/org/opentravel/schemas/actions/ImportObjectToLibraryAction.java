...
public class ImportObjectToLibraryAction extends OtmAbstractAction {
    ...
    /**
     * Import/Copy selected components into the destination library. User is asked about type
     * assignment changes. Used by DND and right-click copy operations.
     * 
     * @param destination
     */
    public void importSelectedToLibrary(final LibraryNode destination) {
        ...
        if (!destination.isTLLibrary()) {
            ...
            DialogUserNotifier.openInformation("WARNING",
                    "You can not import into a built-in or XSD library.");
            return;
        }
        ...
        int ans = DialogUserNotifier.openQuestionWithCancel(
                Messages.getString("action.import.copyDescendants.title"),
    ...
    /**
     * Change the existing sourceNode type assignments to use newNode. Only writable nodes are
     * changed. The user is asked if they want the change to be global, local or none. Properties of
     * the newNode, if they exist, are always done.
     * 
     * @param sourceNode
     * @param newNode
     */
    public GlobalDialogResult askGlobalLocalNone(final List<Node> sourceNodes,
            final LibraryNode destLib) {
