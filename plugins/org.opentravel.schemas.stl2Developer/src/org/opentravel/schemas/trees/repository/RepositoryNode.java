...
public abstract class RepositoryNode extends Node implements Comparable<RepositoryNode> {
		...
		protected List<Node> initChildren() {
			...
			for (String baseName : getBaseNamespaces()) {
				if (baseName.startsWith(getName())) {
					BaseNamespaceNode bs = new BaseNamespaceNode(baseName, getRootBasename(), getRepository());
	...
	public static class BaseNamespaceNode extends NamespaceNode {
		...
		protected List<Node> initChildren() {
			...
			for (Entry<RepositoryItem, List<RepositoryItem>> e : libItems.entrySet()) {
				RepositoryChainNode ri = new RepositoryChainNode(getRepository(), e.getKey(), e.getValue());
				ri.setParent(this);
				namespaces.add(ri);
			}
			return namespaces;
		...
		private static Map<RepositoryItem, List<RepositoryItem>> filterLibItems(List<RepositoryItem> items2) {
	...
	public static class RepositoryChainNode extends RepositoryNode {
		...
		public RepositoryChainNode(Repository repository, RepositoryItem head, List<RepositoryItem> items) {
			super(repository);
			this.head = head;
			this.items = items;
		}
