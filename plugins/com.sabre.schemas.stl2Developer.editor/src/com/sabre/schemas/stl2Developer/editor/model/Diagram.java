...
public class Diagram implements LayoutListener {
    ...
    /**
     * Calculate best possible position newChild and calculate new position based on existing
     * location and {@link Position}.
     * 
     * @param existing
    ...
    public Point findBestLocation(UINode existing, Node newChild, Position posistion) {
        ...
        if (closeOne != null) {
            ...
            if (posistion == Position.RIGHT) {
                ...
                newLocation = existing.getBoundry().getTopLeft()
                        .translate(-getFigureSize(newChild).width - 70, 0);
            }
        }
    ...
    /**
     * Add temporary GEP in order to get the figure size. This method is cleaning up every EditParts
     * and ConnectionEditPArt after it finish.
     * 
     * @param newChild
     * @return
     */
    private Dimension getFigureSize(Node newChild) {
        ...
        GraphicalEditPart gep = (GraphicalEditPart) getViewer().getEditPartFactory()
                .createEditPart(ownerEP, uiModel);
            ...
            if (pos == Position.RIGHT) {
                if (referancePoints.x - shape.getLocation().x < 0) {
                    double newMin = Math.min(
                            referancePoints.getDistance(shape.getBoundry().getBottomLeft()), min);
                    if (min > newMin) {
                        min = newMin;
                        minNode = shape;
                    }
