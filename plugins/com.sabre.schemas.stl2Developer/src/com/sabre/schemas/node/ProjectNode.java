...
public class ProjectNode extends Node implements INode {
    ...
    /**
     * Create a project node from the TL project model. Read all libraries in the project and create
     * library nodes and their children. Link into the model node.
     * 
     * @param tlProject
    ...
    /**
     * Link the library as a child to the project node. Does NOT add to the underlying model. Does
     * not resolve types.
     * 
     * @param lib
     */
    ...
    /**
     * Load the GUI model with all unmodeled libraries from the TL Project into this project.
     * Resolves types after all the libraries are loaded.
     */
    ...
    /**
     * Add the files to the project and models all new project items in the GUI model. NOTE - for
     * performance reasons, always try to add multiple files at once.
     */
    ...
    public String getLabel() {
        if (OtmRegistry.getMainController() != null
                && OtmRegistry.getMainController().getProjectController() != null) {
            ProjectController pc = OtmRegistry.getMainController().getProjectController();
    ...
    /**
     * @return the portion of the project namespace that is managed/governed by a repository. Empty
     *         string if not managed.
     */
    public String getNSRoot() {
        String ns = getNamespace();
        for (String managedRoot : OtmRegistry.getMainController().getRepositoryController()
                .getRootNamespaces())
    ...
    /**
     * @return the portion of the project namespace that is not managed/governed by a repository.
     *         Returns the entire namespace if unmanaged.
     */
    ...
    // /* (non-Javadoc)
    // * @see com.sabre.schemas.node.INode#isTypeUser()
    // */
    // @Override
    ...
    /**
     * Get the chain that the project item should belong to. Null if no chains with same base
     * namespace are open.
     * 
     * @param pi
     * @return
