...
public class NodeNameUtils {
    ...
    /**
     * Assure the indicator name has the proper suffix, not the banned prefix (is) and initial lower
     * case.
     * 
     * @param name
     * @return
    ...
    /**
     * Assure the indicator element name has the proper suffix, not the banned prefix (is) and
     * initial cap.
     * 
     * @param name
     * @return
    ...
    /**
     * Split a string looking like {urn:scim:schemas:core:1.0}Status as created by qName.toString()
     * back into a qname. If no namespace, the chameleon namespace constant used by the compiler is
     * used.
     * 
     * @param qname
     * @return
    ...
    private static Collection<PropertyNodeType> lowerCasePropertyTypes() {
        return EnumSet.of(PropertyNodeType.ATTRIBUTE, PropertyNodeType.INDICATOR,
                PropertyNodeType.ID);
    }
    ...
    /**
     * Fix an XSD type name. Remove "type" or "Type" and assure initial cap DO NOT USE when a type
     * has been assigned. Use {@link #fixElementName(Node)} instead.
     * 
     * @param name
     *            - an xsd type QName
    ...
    /**
     * Get the correct element name based on the type assigned to the node. If the type will be
     * assigned in the XSD schema by type reference, then the global element name must be used. The
     * codeGenUtils know which types are done by reference.
     * 
