 ...
 * Attributes and others. (See PropertyNodeType: Role, Literal, Alias, Simple)
 * 
 * Properties extends components by giving them the ability change nature (Indicator, attribute,
 * element) without losing data unique to each one. To do this, it has extra model objects. Also
 * used for RoleProperty nodes.
 * 
...
public class PropertyNode extends ComponentNode {
     ...
     * @param propertyType
     */
    protected PropertyNode(final TLModelElement tlObj, INode parent,
            final PropertyNodeType propertyType) {
        super(tlObj);
     ...
     */
    protected PropertyNode(final TLModelElement tlObj, final Node parent, final String name,
            final PropertyNodeType type) {
    ...
    public boolean canAssign(Node type) {
        ...
        switch (getPropertyType()) {
            ...
            case INDICATOR_ELEMENT:
                return (type == ModelNode.getIndicatorNode() || type == ModelNode
                        .getUndefinedNode());
    ...
    /**
     * Property Roles are displayed in the facet table and describe what role the item can play in
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.Node#getType()
     */
    @Override
    public Node getType() {
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.Node#getOwningComponent()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.Node#isMissingAssignedType()
     */
    @Override
    ...
    /**
     * Index of this property in the TL Model object. The TL Model keeps separate lists for
     * elements, attributes and indicators.
     * 
    ...
    /**
     * Node index is the order in the node list. Node lists are not separated by node types as they
     * are in TL Model objects.
     * 
     * @return
     */
    public int indexOfNode() {
        int index = getParent().getChildren().indexOf(this) + 1;
        return ((index < 1) || (index > getParent().getChildren().size())) ? index = getParent()
    ...
    /**
     * If <i>this</i> property does not have a type, assign it to the source node. Otherwise, create
     * a new property after <i>this</i>node. Use the passed node as the type.
     * 
    ...
    public boolean hasNavChildrenWithProperties() {
        return isProperty() && !(this instanceof IndicatorNode) && !isEnumerationLiteral()
                && !isRoleProperty() && modelObject != null && modelObject.getTLType() != null;
    }

    /*
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeUser#getAssignedType()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeProvider#getAssignedModelObject()
     */
    @Override
    public ModelObject<?> getAssignedModelObject() {
        return getTypeClass().getTypeNode() != null ? getTypeClass().getTypeNode().getModelObject()
                : null;
    }
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeProvider#getAssignedTLObject()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.Node#sort()
     */
    @Override
    public void sort() {
        getParent().sort();
    ...
    /**
     * Replace this property with one of the specified type. Uses the saved alternateRole or creates
     * a new property. All values that can be copied will be. The old property is saved.
     * 
    ...
    /**
     * Make this have same values as source node (name, mandatory, doc, ex, eq). Does <b>not</b> set
     * types.
     * 
     * @param target
     */
    ...
    /**
     * A property within a facet can change roles. This class keeps track of the various
     * implementations that may be used for any property. All properties that can be alternated
     * between should share this class.
     * 
     * @author Dave Hollander
     * 
     */
    protected class AlternateRoles {
        ...
        /**
         * If an old copy of this property has been saved, return it; otherwise create a new
         * property. New properties are saved.
         * 
         * @param type
         * @param propertyNode
         * @return property or null if not supported for the property type or this was already that
         *         type.
         */
        public PropertyNode oldOrNew(PropertyNodeType type) {
            ...
            switch (type) {
                case ELEMENT:
                    ...
                    break;
                case ID_REFERENCE:
                    pn = oldEleRefN != null ? oldEleRefN : new ElementReferenceNode(getParent(),
                            getName());
                    oldEleRefN = (ElementReferenceNode) pn;
                    break;
                case ATTRIBUTE:
                ...
                case INDICATOR:
                    ...
                    break;
                case INDICATOR_ELEMENT:
                    pn = oldIndEleN != null ? oldIndEleN : new IndicatorElementNode(getParent(),
                            getName());
                    oldIndEleN = (IndicatorElementNode) pn;
                    break;
                case ID:
