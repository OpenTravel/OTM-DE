...
import com.sabre.schemas.modelObject.XSDElementMO;
import com.sabre.schemas.modelObject.XSDSimpleMO;
...
public abstract class Node implements INode {
    ...
    /**
     * Within the node classes, public adders and setters are responsible for keeping the nodes and
     * underlying library model in sync.
     * 
     */
    ...
    /**
     * Create a node containing modelObject for the tlObject and assign name and description fields.
     * Can be used for LibraryMember elements which are top level members of the library.
     * 
     * The model object will NEVER be null. It may contain an EmptyMO. Model object factory links in
     * the TLModelElement and sets edit-able flag
     */
    ...
    /**
     * Create a model object for the passed TLModel element and back link from the modelObject to
     * this node.
     * 
     * @param obj
    ...
    /**
     * Public class for comparing nodes. Use: Collection.sort(list, node.new NodeComparable()) Uses
     * node name and prefix in the comparison.
     */
    ...
    protected void removeChild(final Node n) {
        ...
        if (!getChildren().contains(n)) {
            ...
            // deleted.
            LOGGER.warn("Attempting to delete a child " + n.getName()
                    + " that is not in children list of parent " + this.getName());
            return;
        }
        children.remove(n);
    }
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#removeFromLibrary()
     */
    @Override
    ...
    /**
     * Swap the replacement for this node. This node is replaced via {@link #replaceWith()} and
     * removed from parent. Replacement is added to parent.
     * 
     * @return Returns false if the swap was not made or the replacement could not be assigned. If
     *         false, the swapped node may still be used as a type but may not be in a library.
     */
    ...
    /**
     * Replace nodes, assigned types and library tree structures. Replaces node in the library then
     * uses {@link #replaceWith(Node)} to replace assignments.
     * 
     * Does <b>not</b> delete this node. <i>Does</i> remove parent and library links. <i>Does</i>
     * remove type usage links to this node.
     * 
     * @param replacement
     */
    ...
    /**
     * Replace all type assignments to this node with assignments to passed node. For every
     * assignable descendant of sourceNode, find where the corresponding sourceNode children are
     * used and change them as well. See {@link #replaceWith(Node)}.
     * 
    ...
    /**
     * Delete a list of nodes. The list can contain any combination of nodes so care must be taken
     * to prevent concurrent modification. Note: facet nodes are NOT deleted except custom facets.
     * 6/26 - removed null parent test. 627 - re-added it
     */
    ...
    /**
     * Find the first node in the descendants of this node with the given name. The order searched
     * is not guaranteed. Will not find family nodes.
     * 
     * @param name
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getChildren()
     */
    @Override
    ...
    /**
     * Gets the descendants that are type users (can be assigned a type). Does not return navigation
     * nodes. {@link #getChildren_TypeUsers() Use getChildren_TypeUsers() for only immediate
     * children.}
     * 
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getModelObject()
     */
    @Override
    ...
    /**
     * Get the TL model object from the node's model object or null if there is no underlying model
     * object or tl model object.
     * 
     * @return
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getName()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getNamePrefix()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getNamespace()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getNameWithPrefix()
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getParent()
     */
    ...
    /**
     * Get the type class for this node. This should <b>not</b> be used except when creating a node
     * set representing an existing TL library.
     * 
    ...
    /**
     * Get a list of the children to be used for navigation purposes. Each node class and model
     * object class determines which children are navigation, if any.
     * 
     * @return
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.Node#getChildren_TypeProviders()
     */
    @Override
    ...
    /**
     * Gets the children that are type users (can be assigned a type). Does not return navigation
     * nodes. {@link #getDescendants_TypeUsers() Use getDescendants_TypeUsers() for all children.}
     * 
    ...
    /**
     * Gets all the types assigned to this type and all the types assigned to those types, etc.
     * Sends back a list of unique types. Types used recursively are only added to the list once.
     * 
    ...
    private HashSet<Node> getDescendants_AssignedTypes(boolean currentLibraryOnly,
            ...
            if (n.getAssignedType() != null) {
                ...
                if (!currentLibraryOnly || (assignedType.getLibrary() == getLibrary()))
                    if (foundTypes.add(assignedType)) {
                        foundTypes.addAll(assignedType.getDescendants_AssignedTypes(
                                currentLibraryOnly, foundTypes));
                    }
            }
        }
        return foundTypes;
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#hasChildren()
     */
    @Override
    ...
    /**
     * @return true if there are children that are properties that can be assigned a type. Does not
     *         include indicators, enumeration literals, or roles Does not include properties whose
     *         model object or TL Type are NULL. (modelObject != null && modelObject.getTLType() !=
     *         null;)
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#isLibraryContainer()
     */
    @Override
    ...
    /**
     * Extensible objects have the ability to create extension points when compiled into schemas.
     * These include core and business objects as well as operations and extension points.
     * {@link #isExtensible()}
     * 
    ...
    /**
     * Enable the extend-able property for this object. For faceted objects, this instructs compiler
     * to create extension points. For enumerations, this creates an open enumeration.
     * 
     * @return - returns this node, or the created open/closed enumeration node
     *         {@link #isExtensible()}
     */
     ...
     * Extensible objects have the ability to create extension points when compiled into schemas.
     * These include core and business objects as well as operations and extension points.
     * {@link #isExtensibleObject()}
     * 
     * @return true if this object has the characteristic of being extensible and the model object
     *         is set to create extension points on compile.
     */
    ...
    /**
     * @return False for node that can not be deleted: not-editable, facets, simpleFacets. Custom
    ...
    public boolean isXSD_Atomic() {
        return getTypeClass().getTypeNode() instanceof ImpliedNode ? ((ImpliedNode) getTypeClass()
                .getTypeNode()).getImpliedType().equals(ImpliedNodeType.XSD_Atomic) : false;
    }
    ...
    /**
     * @return true if the node represents a non-imported XSD type. Note - the library is a
     *         TLLibrary, but the type is not editable.
     */
     ...
     */
    /**
     * @return - true if the node object is either open or closed enumeration ??? is enumeration
     *         always a facetNode?
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#isTypeProvider()
     */
    @Override
    ...
    /**
     * @return true if a named top level object (business, core, enum, vwa, simple, extension point
     *         facet)
     */
     ...
     * 
     *         Note: if parent is not known, attributes are assumed to not be part of a
     *         ValueWithAttribute and therefore are simpleTypeUsers
     * 
    ...
    /**
     * Returns true if the node name and namespace is unique compared against all of <i>this</i>
     * children. If the child is in a query facet, then the test is only across that facet.
     * 
     * @param test
     *            node to test, <i>this</i> node to check children of. If property or facet, will go
     *            to parentNode to start check.
     * @return true if unique
     * 
     *         FIXME - this assumes that we are only comparing properties of complex objects Will
     *         not work for nav nodes
     */
    ...
    /**
     * Simply link in the child node to <i>this</i> node at the index location. No family match test
     * is made. Assures the child is not already in the list.
     * 
     * @param child
     *            - node to add to <i>this</i>
     * @param index
     *            - integer ordinal value where to add the child if in range. If out of range, added
     *            to end.
     */
    ...
    /**
     * Add the <i>child</i> node parameter to <i>this</i> node. Sets parentNode link of child. NOTE
     * - must have a name to support family processing.
     * 
     * @param child
     *            - node to be added
     * @param doFamily
     *            - if present and true, the child is added to the a family node if the name matches
     *            family conditions. If child.family is not set it will make a family name. Checks
     *            all the children of this node to see if the child should be added to the family
     *            instead of directly to this node.
    ...
    /**
     * Add <i>this</i> child to a family node for it and its "peer" node. If the peer is a Family
     * node then just add it, otherwise create the family node.
     * 
     * @return
     */
    ...
    /**
     * Adds the passed node as a child of <i>this</i> node, if its name and namespace are unique
     * amongst the children. Used to link properties to facets.
     * 
     * @param child
     *            - node to link in
    ...
    /**
     * Set the name in the node and underlying model object for <i>this</i> node. If this node has
     * parentNode and grandparent, unlink and re-link to execute family logic. Overridden in complex
     * type nodes to propagated name change type users. NOTE - not safe to use in constructors until
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#setName(java.lang.String, boolean)
     */
    @Override
    ...
    /**
     * Unlink this node from its parent and updates family node as needed. If this is a versioned
     * object, it unlinks the verionNode as well. Does not change the TL model. Does not delete the
     * node; caller is responsible to free resources.
     * 
     */
    public void unlinkNode() {
        ...
        if (!parent.children.remove(this))
            LOGGER.debug("unlinkNode Error - child " + getName() + " was not in parent's "
                    + parent.getName() + " child list.");
        parent.updateFamily();
        parent = null;
    ...
    public List<Node> setTLTypeOfTargetChildren(List<Node> targets, Node replacement) {
        ...
        for (Node t : targets) {
            ...
            if ((t.getParent() == replacement) || (t.getParent().getParent() == replacement)) {
                ...
                LOGGER.debug("TODO - TEST - setTLTypeOfTargetChildren() - This seems all wrong. t = "
                        + t.getName());
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#setAssignedType(com.sabre.schemas.node.INode )
     */
    @Override
    ...
    /**
     * Return the node used as the assigned type. NOTE: does not return node assigned as base types
     * for core and business objects.
     * 
     * @return
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getType()
     */
    @Override
    public Node getType() {
        return (this instanceof TypeUser) ? getAssignedType() : null;
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#getTypeUsers()
     */
    @Override
    ...
    /**
     * Set the extension base to the passed source node. If null, remove assignment. Extension base
     * is maintained in the TypeClass.typeNode.
     * 
     * @param sourceNode
     */
    ...
    public boolean isInstanceOf(Node node) {
        if (isExtendedBy(node)) {
            ...
            if (baseNode == null) {
                LOGGER.warn("Could not find the base node: [" + getExtendsTypeNS() + ":"
                        + getExtendsTypeName() + "]");
                return false;
            }
     ...
     * ************************ Documentation ****************************
     * 
     * TODO - reconcile with the documentation view use of the documentation nodeManager. TODO - do
     * the enum/string[] right
     */
    public static enum DocTypes {
        Description, Deprecation, MoreInformation, Implementer, ReferenceLink
    };

    public static String[] docTypeStrings = { "Description", "Deprecation", "MoreInformation",
            "Implementer", "ReferenceLink" };
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.node.INode#isDeprecated()
     */
    @Override
    ...
    /**
     * Change all context users to use targetId. Then
     * 
     * @param contextId
    ...
    /**
     * NOTE - this does not detect nodes that are created by the xsd utilities that represent local
     * or anonymous types. Use {@code isXsdType()} instead.
     * 
    ...
    /**
     * Get all libraries under <i>this</i> node. Note - only searches library containers. Libraries
    ...
    /**
     * Get all user libraries (OTM TLLibrary) under <i>this</i> node. Note - only searches library
     * containers. Libraries in the tree with an ancestor that is not a library container will not
     * be found.
     * 
     * @return new list of library nodes.
     */
    ...
    /**
     * Return the single node that represents this object. Null if none. For VWA and Core, the
     * simple property node is returned.
     * 
     * @return
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeUser#getAssignedModelObject()
     */
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeUser#getAssignedTLObject()
     */
    // @Deprecated
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.types.TypeUser#removeAssignedType()
     */
    ...
    /**
     * Clone a node. Clone this node and all of its children. Creates new ModelObject, TL source
     * object. Sets types for all the properties. Creates type node. Types are assigned to this
     * component. Clones the TL and Model objects. Must be a library member. Assigns libraries and
     * types. Added to parent and a family node may be created to contain <i>this</i> and the clone.
     * 
     * Note: the new component is <b>not</b> used to replace type users of this node (see {@link
     * replaceTypesWith()}
     * 
     * @param library
    ...
    /**
     * Clone this node. If parent is null, the new node is only added to this library. If the parent
     * is a library then the new node is added to <i>this</i> node's parent adjacent to this node if
     * a property. If this is a namedMember() then the clone is added to this.library. Otherwise,
     * parent is used to contain the new node.
     * 
     * @param parent
     * @param nameSuffix
     * @return
    ...
    public Node clone(Node parent, String nameSuffix) {
        ...
        if (this instanceof PropertyNode) {
            ...
            if (parent != null && parent instanceof ComponentNode) {
                ((ComponentNode) parent).addProperty(newNode,
                        ((PropertyNode) this).indexOfTLProperty());
            }
    ...
    /**
     * Set all the type users in newNode to match this nodes assignments. Only does typeClass
     * assignments, not TL model assignments.
     * 
     * @param newNode
     */
    ...
    public ValidationFindings validate() {
        ValidationFindings findings = TLModelCompileValidator.validateModelElement(this
                .getTLModelObject());
    ...
    public Document compileExampleDOM() {
        ...
        try {
            ...
            for (String finding : e.getFindings().getAllValidationMessages(
     ...
     * Visitors *********************************************
     * 
     * Sample Usage: thisModel.getModelNode().visitAllNodes(this.new count()); where: public class
     * count implements NodeVisitor {
     * 
