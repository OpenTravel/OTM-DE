 ...
 * inheritance structure are to be used in the extended entity.
 * 
 * @author S. Livezey
 */
public class ExtensionInheritancePage extends WizardPage implements TypeSelectionListener {
    ...
    protected ExtensionInheritancePage(final String pageName, final String title,
            String description, final ImageDescriptor titleImage, final Node n) {
        ...
        if (curNode != null) {
            ...
            if (modelObject.getTLModelObj() instanceof TLModelElement) {
                ...
                inheritanceHierarchyInfo = new InheritanceHierarchyBuilder(tlObj.getOwningModel())
                        .buildHierarchyInfo();
            }
        }
    }
    ...
    protected ExtensionInheritancePage(final String pageName, final String title,
            String description, final Node n) {
    ...
    public void doPerformFinish() {
        for (PropertyNode property : originalOptions.keySet()) {
            ...
            if (isInheritedProperty(property)) {
                ...
                // declared property
                LOGGER.info("  Creating Property to Eclipse Inherited Property: "
                ...
                if (!isInheritedPropertyType(selectedOption)) {
                    ...
                    // different type
                    LOGGER.info("  Creating Property to Replace the Deleted Property: "
                            + selectedOption.getName());
                    FacetNode currentNodeFacet = getCurrentNodeFacet(property);
    ...
    /**
     * Returns false if the given node is a child (ancestor) of the current node. If the node was
     * inherited from the extension node, true will be returned.
     * 
     * @param propertyNode
     *            the node to analyze
     * @return boolean
    ...
    /**
     * Returns true if the given property type is utilizes by one of the properties on the selected
     * extension node.
     * 
     ...
     * 
     * Attempts to locate a facet on the node that is currently being edited by the user (the one
     * for whom an extension is being chosen). This is necessary because the given property node may
     * belong to the base type that is being extended.
     * 
     * @param propertyNode
    ...
    private FacetNode getCurrentNodeFacet(PropertyNode propertyNode) {
        ...
        if (curNode.getModelObject().getTLModelObj() instanceof TLFacetOwner) {
            ...
            currentNodeFacet = FacetCodegenUtils.getFacetOfType(currentNodeTLObj,
                    propertyOwningFacet.getFacetType(), propertyOwningFacet.getContext(),
                    propertyOwningFacet.getLabel());
        }
    ...
    public boolean notifyTypeSelected(Node selectedExtension) {
        ...
        // && !selectedExtension.getModelObject().isExtensionPointFacet()) {
            ...
            for (PropertyNode propertyNode : propertyNodes) {
                ...
                if ((inheritanceHierarchy != null) && (inheritanceHierarchy.size() > 1)) {
                    ComponentNode selectedOption = getInitiallySelectedOption(propertyNode,
        ...
        if (!extensionProperties.isEmpty()) {
            PropertyNode fieldTableSelection = (PropertyNode) fieldTable.getTable().getItem(0)
                    .getData();
    ...
    private List<PropertyNode> getExtensionProperties(Node extensionNode) {
        ...
        for (PropertyNode node : inheritedProperties) {
            ...
            if ((inheritanceHierarchy != null)
                    && !inheritanceHierarchies.contains(inheritanceHierarchy)) {
                extensionProperties.add(node);
                inheritanceHierarchies.add(inheritanceHierarchy);
            }
        }
    ...
    private void getPropertyNodes(Node node, boolean includeInheritedNodes,
    ...
    private List<ComponentNode> getInheritanceOptions(PropertyNode propertyNode) {
        ...
        if (propertyNode.getPropertyType() == PropertyNodeType.ELEMENT) {
            ...
            List<NamedEntity> tlHierarchyOptions = inheritanceHierarchyInfo.get(tlProperty
                    .getType());
    ...
    public void createControl(final Composite parent) {
        ...
        leftTopPanel.setLayoutData(new GridData(GridData.FILL_BOTH | GridData.GRAB_HORIZONTAL
                | GridData.GRAB_VERTICAL));
        ...
        fieldSelectionTable = new TableViewer(leftBottomPanel, SWT.BORDER | SWT.FULL_SELECTION
                | SWT.SINGLE);
        ...
        fieldSelectionTbl.setLayoutData(new GridData(GridData.FILL_HORIZONTAL
    ...
    private void handleFieldSelectionChanged() {
        ...
        PropertyNode selectedProperty = (selectedItem.length == 0) ? null
                : (PropertyNode) selectedItem[0].getData();
    ...
    /**
     * Initializes the node structure for trees to be displayed within this wizard page. This must
     * be done differently than the normal tree views because this page's view is designed to show a
     * "what-if" scenario based upon the user's current selections.
     */
    ...
    /**
     * Runs at wizard startup. Recursive method that creates and links the children of the given
     * node down to and including the properties-level of the tree.
     * 
     * @param node
    ...
    private void createTreeNodeChildren(ComponentNode node) {
        if (node.getTLModelObject() instanceof TLFacet) {
            ...
            TLFacet baseFacet = (selectedExtensionTLObj == null) ? null : FacetCodegenUtils
                    .getFacetOfType((TLFacetOwner) selectedExtensionTLObj, facet.getFacetType(),
            ...
            if (baseFacet != null) {
                for (TLAttribute attribute : PropertyCodegenUtils
                        .getInheritedFacetAttributes(baseFacet)) {
                    NodeFactory.newComponentMember(node, attribute);
                }
                for (TLIndicator indicator : PropertyCodegenUtils
                        .getInheritedFacetIndicators(baseFacet)) {
                    NodeFactory.newComponentMember(node, indicator);
                }
            }
            ...
            if (baseFacet != null) {
                for (TLProperty property : PropertyCodegenUtils
                        .getInheritedFacetProperties(baseFacet)) {
                    TLPropertyType propertyType = property.getType();
                    Object inheritanceHierarchy = inheritanceHierarchyInfo.get(propertyType);
        ...
        synchronized (fieldSelectionButtons) {
            if (!fieldSelectionButtons.isEmpty()) {
                throw new IllegalStateException(
                        "Field selection radio buttons already initialized.");
            }
     ...
     * Removes the font/color formatting from the default application label provider.
     */
    private class FieldSelectionTableLabelProvider extends FieldTableLabelProvider implements
            ITableLabelProvider {
    ...
    /**
     * Content provider that populates the tree with nodes based on the selections made by the user.
     * This is essentially a "what-if" that allows the user to browse the type/properties tree of
     * the current node before actually committing to the current selections.
     */
                ...
                if (node.isProperty()) {
                    ...
                    if (elem != null) {
                        if (elem instanceof LibraryMember) {
                            return new Object[] { NodeFactory
                                    .newComponent_UnTyped((LibraryMember) elem) };
