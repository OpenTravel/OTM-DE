...
import com.sabre.schemas.node.SimpleFacetNode;
import com.sabre.schemas.node.SubType;
...
import com.sabre.schemas.wizards.ChangeWizard.ExtentedTLFacetType;
...
public class ChangeWizardPage extends WizardPage {
    ...
    protected ChangeWizardPage(final String pageName, final String title,
            final FormValidator validator, final ComponentNode editedNode,
            final List<SubType> allowedObjectTypes,
    ...
    private void setObjectType(final SubType st) {
        LOGGER.debug("setObjectType(" + st + ") " + editedNode.getName() + " has "
                + editedNode.getTypeUsersCount() + " users.");

        final HistoryItem item = new HistoryItem(OpType.OBJECT_TYPE_CHANGE, editedNode, null);
        historyPush(item);
        try {
            editedNode = changeObject(editedNode, st);
            // OtmRegistry.getMainController().checkModelCounts(editedNode);
            tablePoster.postTable(editedNode);
            updateFacetTypeButtons();
            LOGGER.debug("After change " + editedNode.getName() + " has "
                    + editedNode.getTypeUsersCount() + " users.");
        } catch (Exception ex) {
            undoLastOp();
            LOGGER.warn("Error on chaning type to: " + st.toString());
            DialogUserNotifier.openError("Error",
                    "Operation finished with error. Check logs for more information.");
        }
    }

    private ComponentNode changeObject(ComponentNode editedNode, SubType st) {
        switch (st) {
            case BUSINESS_OBJECT:
                return editedNode.changeToBusinessObject();
            case CORE_OBJECT:
                return editedNode.changeToCoreObject();
            case VALUE_WITH_ATTRS:
                return editedNode.changeToVWA();
            default:
                throw new IllegalArgumentException("SubType: " + st.toString()
                        + " is not supporeted.");
        }
    }
        ...
        if (history.size() > 0) {
            ...
            switch (item.opType) {
                ...
                case OWNING_FACET_CHANGE_TO_SIMPLE:
                    ...
                    ComponentNode simpleFacet = ((ComponentNode) item.previousNode.getParent())
                            .getSimpleFacet();
    ...
    private Table createPreviewTable(final Composite container) {
        final TableViewer viewer = new TableViewer(container, SWT.MULTI | SWT.H_SCROLL
    ...
    private void setLibrary(final String selected) {
        ...
        if (!editedNode.getLibrary().equals(selectedLib)) {
            final HistoryItem item = new HistoryItem(OpType.LIB_CHANGE, editedNode.getLibrary(),
                    selectedLib);
            historyPush(item);
    ...
    private void setOwningFacet(final ExtentedTLFacetType st) {
        ...
        for (final PropertyNode property : properties) {
            if (property.getParent() instanceof FacetNode) {
                ...
                LOGGER.warn("Trying to change facets for property " + property
                        + " but parent is not a facet.");
                continue;
            }
            ...
            if (owningFacet.getFacetType() == null
                    || owningFacet.getFacetType().equals(st.toTLFacetType())) {
    ...
    // TODO - use FacetNode
    private void changeFacet(final PropertyNode property, final ComponentNode oldFacet,
            final ComponentNode newFacet) {
        if (newFacet != null) {
    ...
    private void changeToSimple(final PropertyNode property, final ComponentNode oldFacet,
        ...
        if (simpleFacet != null) {
            ...
            final HistoryItem item = new HistoryItem(OpType.OWNING_FACET_CHANGE_TO_SIMPLE,
                    oldFacet, property, clone);
            historyPush(item);
        }
    }
    ...
    private void changeFromSimple(final PropertyNode simpleAttr, final FacetNode simpleFacet,
        ...
        if (targetFacet != null) {
            ...
            // list.
            final HistoryItem item = new HistoryItem(OpType.OWNING_FACET_CHANGE_FROM_SIMPLE,
                    simpleAttrClone, newProperty);
            historyPush(item);
        }
    }
