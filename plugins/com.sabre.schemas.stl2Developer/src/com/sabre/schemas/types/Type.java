...
import com.sabre.schemas.node.NodeVisitors;
import com.sabre.schemas.node.NodeVisitors.FixNames;
 ...
 * 
 * Note: all type users are treated the same, base, element and type references. To know if it is a
 * base type assignment, see if it is a simpleType, simpleFacet or extension.
 * 
...
public class Type {
    ...
    public ArrayList<Node> getTypeUsers() {
        if (typeUsers == null) {
            ...
            throw new IllegalStateException("Set type users durning a getUsers for "
                    + getTypeOwner());
        }
        return typeUsers;
    }
    ...
    /**
     * @return a new list of editable nodes where any member of the owning component (aliases,
     *         facets) are is assigned as a type or base type.
     */
     ...
     * @return
     */
    protected List<Node> getWhereUsed(final Node scope, boolean editableOnly,
            boolean includeExtensionBase) {
        ...
        if (target != null) {
            ...
            if (sourceTLType != null && scope.isInTLLibrary()) {
                ...
                if (includeExtensionBase && target.getTLModelObject() instanceof NamedEntity) {
                    if (scope.getModelObject()
                            .isExtendedBy((NamedEntity) target.getTLModelObject())) {
    ...
    /**
     * Replace this provider with unassignedNode for all users of this provider as a type. Also
     * replaces type usage of descendants of this typeOwner node. Also does the TL properties.
     */
    ...
    /**
     * Replace this provider with replacement for all users of this provider as a type. Also
     * replaces type usage of descendants of this typeOwner node. Also does the TL properties. Note
     * - user counts may change when business replace core objects because core is also a valid
     * simple type.
     * 
     * It is OK for the owner to not be in a library. This happens when it is being replaced.
     * 
     * @param replacement
     *            is the TypeProvider to use instead. Must be a typeProvider. skips assignment if
     *            replacement is not compatible with user.
     * @param libraryScope
    ...
    /**
     * Set the type node and add this owner to that user list. Remove typeOwner from
     * typeNode-typeUsers. Restricted use - Does NOT set the type the TLModel type
     * {@link setAssignedType}
     */
    ...
    /**
     * Set assigned type. Sets type or base type as appropriate for the node. Extends
     * setAssignedType() by using the passed source node to get the Qname of the node to find. To
     * set the QName, the TLTypeObject is tried first if not null, otherwise the assigned xsd type
    ...
    public boolean setAssignedTypeForThisNode(Node source, Map<QName, Node> providerMap) {
        ...
        if (target == null) {
            if (source.getTLTypeObject() != null
                    && source.getTLTypeObject().getOwningLibrary() == null) {
        ...
        if (target == null && typeQname != null && !typeQname.getLocalPart().isEmpty()) {
            if (providerMap == null) {
                ...
                if (target == null)
                    ...
                    target = providerMap.get(new QName(XsdModelingUtils.ChameleonNS, typeQname
                            .getLocalPart()));
                if (target == null)
        ...
        if (source.getName().equals("ALG"))
            LOGGER.debug("Type " + target + " assigned to " + source);
        return ret;
    }
     ...
     * method assures there is a target and that the owner is editable. Sets the type class
     * properties as well as the TLModel type If the target represents a complex type (BO, Core,
     * facet, alias) then the owner's name is changed.
     * 
    ...
    private boolean isTLTypeAssigned() {
        ...
        if (ownerTLType == null)
            ...
            ret = (!(ownerTLType.getLocalName().equals("Undefined")) || ownerTLType.getLocalName()
                    .equals("Unassigned-missingAssignment"));
        // if (!ret)
    ...
    public boolean verifyAssignment(Map<QName, Node> providerMap, QName typeQname) {
        ...
        if (!isTLTypeAssigned()) {
            if (isTypeNodeAssigned()) {
                LOGGER.debug("Invalid: " + typeOwner + " TLType is not assigned "
                        + " but TypeNode is assigned: " + typeOwner.getTypeClass().getTypeNode());
                return false;
            } else
                return true;
        }
        ...
        if (!ret)
            if (Node.getModelNode().getDuplicateTypes().contains(typeNode)) {
                ...
                LOGGER.debug("INVALID Assignment to "
                        + typeOwner.getTLModelObject().getValidationIdentity() + " with TL type "
                        + typeOwner.getTLTypeObject().getValidationIdentity());
                LOGGER.debug("    but type node has "
                        + typeNode.getTLModelObject().getValidationIdentity());
                if (providerMap != null) {
                    LOGGER.debug("Map results for " + typeQname + " = "
                            + providerMap.get(typeQname));
    ...
    /**
     * Search the model node for all users of the type owner. Sets the typeUsers array with the new
     * list.
     * 
     ...
     * = null.
     * 
     * Does Change the TL Model. Does Not Change the Model Object. TEST Sets the TL model object
     * type and adds this node TEST the assigned type's users list.
     */
    ...
    /**
     * This owner is removed from assigned type's where-used list, but leaves typeNode and tlType
     * assigned.
     */
    ...
    /**
     * Clear users of the this type provider. Sets all users to unassigned. Intended for use in bulk
     * operations as it does not do children or support limiting scope. Use ReplaceTypeProvider()
     * for general usage.
     */
    ...
    /**
     * Delete the type node and remove all links to and from type users. Unlike {@link clear()},
     * delete does change the type assignments to the underlying TL model.
     */
    public void delete() {
        if (typeNode != null)
            typeNode.typeUsers().remove(typeOwner);
    ...
    /**
     * Deletes users of the this type provider. Sets all users to unassigned. Intended for use in
     * bulk operations as it does not do children or support limiting scope. Use
     * ReplaceTypeProvider() for general usage. This method <b>does</b> remove TL Model type
     * assignments to this type.
     */
