...
import com.sabre.schemas.node.controllers.ContextNodeModelManager;
...
public class ContextModelManager {
    /*
     * The context model has an outer map with the library as the key. The inner map then user the
     * contextID as a key to the Context containing a node and TLContext object.
     * 
    ...
    /**
     * @return the new ContextNode cloned from the selected node, complete with new TLContext which
     *         has been added to TLLibrary
     */
    ...
    /**
     * Create a copy of the passed TLContext and add it as a new context node in the target
     * library's context list. If the context already existed, nothing is done.
     * 
     * @param tlc
     *            - context to add
     * @param targetLib
    ...
    public TLContext getDefaultContext(TLLibrary tlib) {
        ...
        if (tlib == null)
            throw new IllegalArgumentException("No library to create context node into.");
    ...
    public String setContextId(TLContext tlc, String newId) {
        ...
        if (c != null) {
            ...
            for (Context ctx : cMap.values()) {
                if (ctx.ctx.getApplicationContext() != null
                        && ctx.ctx.getApplicationContext().equals(existingAppCtx))
                    removeFirst = true;
    ...
    public String setDefaultContext(LibraryNode ln, String contextID) {
        ...
        if ((contextID != null) && (!contextID.isEmpty())
                && contextMap.get(ln).get(contextID) != null) {
    ...
    /**
     * Search the context map and node tree looking for the passed id string. Debugging Utils
     * private boolean searchForId(String id) { if (id == null || id.isEmpty()) id = ClearedName;
     * boolean ret = false;
     * 
     * // makes sure all cNodes in the map are unique. for (LibraryNode lib : contextMap.keySet()) {
     * for ( Context c : contextMap.get(lib).values()) { if (searchForCNode(c.cNode) > 1)
     * LOGGER.error("Shared cNode! "+c.contextId); } }
     * 
     * 
     * // Context map for (LibraryNode lib : contextMap.keySet()) { for ( String thisId :
     * contextMap.get(lib).keySet()) { // LOGGER.debug("Testing context id of "+thisId); if
     * (thisId.startsWith(id)) { LOGGER.debug("Found context id of "+thisId); ret = true; } } //
     * Look into the TL Library TLLibrary tlLib = lib.getTLLibrary(); for (TLContext tlc :
     * tlLib.getContexts()) { // LOGGER.debug("Testing tl context id of "+tlc.getContextId()); if
     * (tlc.getContextId().startsWith(id)) {
     * LOGGER.debug("Found tl context id of "+tlc.getContextId()); ret = true; } } }
     * 
     * // Check the node tree List<ContextNode> children = root.getChildren(); for (ContextNode cn :
     * root.getChildren()) { LOGGER.debug("");
     * LOGGER.debug("Testing node context library of "+cn.getLabel
     * ()+" - "+cn.getLibraryNode().getTLaLib().getLibraryUrl()); if
     * (cn.getType().equals(ContextNode.ContextNodeType.LIBRARY_ROOT)) { if
     * (cn.getChildren().isEmpty())
     * LOGGER.debug("No children of library node: "+cn.getLabel()+" - "+
     * cn.getLibraryNode().getTLaLib().getLibraryUrl()); for (ContextNode childCN :
     * cn.getChildren()) {
     * LOGGER.debug("Testing node context id of "+childCN.getLabel()+" class = "+childCN); if
     * (childCN.getLabel() == null) { LOGGER.debug("Found NULL context id of "+childCN); } else if
     * (childCN.getLabel().startsWith(id)) {
     * LOGGER.debug("Found context id of "+childCN.getLabel()); ret = true; } } } }
