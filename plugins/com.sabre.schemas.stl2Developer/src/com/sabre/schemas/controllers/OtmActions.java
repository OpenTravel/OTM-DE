...
import com.sabre.schemas.actions.SetObjectNameAction;
import com.sabre.schemas.controllers.DefaultContextController.ContextViewType;
...
public class OtmActions {
    ...
    /**
     * Get the current node or first node selected from facetView. Inform the user if no nodes were
     * selected.
     * 
     * @return
     */
    ...
    public void changePropertyRole(final OtmEventData wd) {
        ...
        if (npn != pn) {
            ...
            DialogUserNotifier.openWarning("No Change",
                    "The property could not be changed as requested.");
    }
    ...
    private void setNameSpace(final OtmEventData wd) {
        ...
        if (curNode != null && curNode instanceof LibraryNode) {
            ((LibraryNode) curNode).getNsHandler().renameInProject(curNode.getNamespace(),
                    wd.getText());
        }
    }
    ...
    private void setExample(final OtmEventData wd) {
        final Node n = (Node) mc.getCurrentNode_PropertiesView();
        final String context = mc.getContextController().getSelectedId(ContextViewType.TYPE_VIEW,
                n.getLibrary());
    ...
    private void setOrNewPropertyType(final OtmEventData ed) {
        ...
        /*
         * If the user did a Control DND or dropped onto a facet or on property with the same type
         * then add a new property. Otherwise, change the type.
         */
    ...
    private void showInvalidTargetWarning() {
        DialogUserNotifier
                .openInformation("WARNING", Messages.getString("dnd.drop.invalid.object"));
    }
    ...
    /**
     * Set the assigned type of the current property posted in the properties view. Event data (in
     * ed.text) is used as the NodeID of the type source node.
     */
