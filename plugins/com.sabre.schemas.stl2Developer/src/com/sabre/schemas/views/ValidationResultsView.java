...
public class ValidationResultsView extends OtmAbstractView {
    ...
    public static String VIEW_ID = "com.sabre.schemas.stl2Developer.ValidationResultsView";
    ...
    public void createPartControl(final Composite parent) {
        ...
        Table findingsTable = new Table(parent, SWT.MULTI | SWT.BORDER | SWT.FULL_SELECTION
    ...
    /**
     * In some cases, validation findings will be reported for entities that do not have
     * corresponding nodes in the GUI. In those cases, the appropriate parent of the finding's
     * source will be returned. If the source of the given finding is already mappable to a GUI
     * node, the finding's source will be returned as-is.
     * 
     * @param finding
    ...
    private ArrayList<Node> findMatching(ValidationFinding finding, String error) {
        ...
        Node n = NodeFinders.findNodeByValidationIentity(finding.getSource()
            ...
            if (f.getMessageKey().contains(error)) {
                for (Object o : f.getMessageParams()) {
                    if (o.equals(finding.getMessageParams()[0])) {
                        ...
                        n = NodeFinders.findNodeByValidationIentity(f.getSource()
                                .getValidationIdentity());
                        if (n != null) {
    ...
    public void copyFindingsToClipboard() {
        ...
        while (iterator.hasNext()) {
            textResults.append(
                    iterator.next().getFormattedMessage(FindingMessageFormat.IDENTIFIED_FORMAT))
                    .append('\n');
        }
    ...
    private void sortTableColumns(int columnIndex) {
        ...
        Collections.sort(findingsContent, new FindingComparator(
                (ITableLabelProvider) findingsViewer.getLabelProvider()));
    ...
    public void validateNode(final Node node) {
        ...
        if (node instanceof ModelNode)
            ...
            findings = TLModelCompileValidator.validateModelElement(((LibraryNode) node)
                    .getTLaLib());
            ...
            for (Node n : node.getDescendants_NamedTypes()) {
                if (findings == null)
                    ...
                    findings.addAll(TLModelCompileValidator.validateModelElement(n
                            .getTLModelObject()));
            }
        } else
        ...
        mc.postStatus("Validation of " + node + " complete, " + findingsCount
                + " warnings and errors found.");
    }
