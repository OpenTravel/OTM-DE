...
import com.sabre.schemas.actions.AddDocItemAction;
import com.sabre.schemas.actions.ClearDocItemAction;
import com.sabre.schemas.actions.CloneDocItemAction;
import com.sabre.schemas.actions.DeleteDocItemAction;
import com.sabre.schemas.actions.DownDocItemAction;
import com.sabre.schemas.actions.NextDocItemAction;
import com.sabre.schemas.actions.PrevDocItemAction;
import com.sabre.schemas.actions.UpDocItemAction;
import com.sabre.schemas.controllers.DefaultDocumentationController;
import com.sabre.schemas.controllers.DocumentationController;
...
import com.sabre.schemas.trees.documentation.DocumentationTreeContentProvider;
import com.sabre.schemas.trees.documentation.DocumentationTreeLabelProvider;
...
public class DocumentationView extends OtmAbstractView implements ISelectionListener,
    ...
    public static String VIEW_ID = "com.sabre.schemas.stl2Developer.DocumentationView";
    ...
    public void createPartControl(Composite parent) {
        ...
        addDocItemAction = new AddDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.addDocItem"));
        cloneDocItemAction = new CloneDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.cloneDocItem"));
        deleteDocItemAction = new DeleteDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.deleteDocItem"));
        upDocItemAction = new UpDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.upDocItem"));
        downDocItemAction = new DownDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.downDocItem"));
        clearDocItemAction = new ClearDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.clearDocItem"));
        ...
        prevDocItemAction = new PrevDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.prevDocItem"));
        nextDocItemAction = new NextDocItemAction(mainWindow, new ExternalizedStringProperties(
                "action.nextDocItem"));
        ...
        docText = toolkit.createText(compRight, "", SWT.MULTI | SWT.BORDER | SWT.WRAP
                | SWT.V_SCROLL);
    ...
    private TreeViewer initializeTreeViewer(Composite parent) {
        ...
        viewer.addDragSupport(DND.DROP_COPY | DND.DROP_MOVE,
                new Transfer[] { TextTransfer.getInstance() }, new DocDragSourceListener(viewer));
        viewer.addDropSupport(DND.DROP_COPY | DND.DROP_MOVE,
                new Transfer[] { TextTransfer.getInstance() }, new ViewerDropAdapter(viewer) {
    ...
    private DocumentationNode generateDocumentationTree(final INode node) {
        ...
        if (model != null && model.isDocumentationOwner()) {
            root = new DocumentationNodeModelManager().createDocumentationTreeRoot(model
                    .getDocumentation());
        }
        return root;
    }
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.jface.viewers.ITreeViewerListener#treeCollapsed(org.eclipse.jface.viewers.
     * TreeExpansionEvent)
     */
    @Override
     ...
     * (non-Javadoc)
     * 
     * @see org.eclipse.jface.viewers.ITreeViewerListener#treeExpanded(org.eclipse.jface.viewers.
     * TreeExpansionEvent)
     */
    @Override
        ...
        if (root != null) {
            for (DocumentationNode n : root.getChildren()) {
                if ("Description".equals(n.getLabel())) {
                    DocumentationNode desc = n.getChildren().size() > 0 ? n.getChildren().get(0)
                            : null;
                    select(desc);
                    return;
                }
