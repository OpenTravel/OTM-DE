...
import com.sabre.schemas.actions.ChangeObjectAction;
import com.sabre.schemas.actions.ClearExtendsAction;
import com.sabre.schemas.actions.CloneSelectedFacetNodesAction;
import com.sabre.schemas.actions.DownFacetAction;
import com.sabre.schemas.actions.ExtendableAction;
import com.sabre.schemas.actions.ExtendsAction;
import com.sabre.schemas.actions.IWithNodeAction;
import com.sabre.schemas.actions.UpFacetAction;
import com.sabre.schemas.commands.AddNodeHandler;
import com.sabre.schemas.commands.DeleteNodesHandler;
import com.sabre.schemas.commands.GoToTypeHandler;
...
import com.sabre.schemas.widgets.ButtonBarManager;
import com.sabre.schemas.widgets.ButtonWithAction;
import com.sabre.schemas.widgets.LibraryTablePoster;
import com.sabre.schemas.widgets.LibraryTablePosterWithButtons;
...
/**
 * Create and maintain the facet table panel. Posts content into the table. Maintains a current and
 * previous node cursor. Maintains enabled/disabled state on the buttons.
 * 
 * @author Dave Hollander
 */
public class FacetView extends OtmAbstractView {
    public static String VIEW_ID = "com.sabre.schemas.stl2Developer.FacetView";
    ...
    protected void initialize(final Composite facetComposite) {
        ...
        mc.getHandlers().enableDropTarget(table, mc.getActions(),
                OtmActions.setOrNewPropertyType(), mc.getWidgets());
    }
    ...
    /**
     * Return a node list of all the items selected or checked in the table. Warning - changes
     * selection to include checked items.
     * 
     * @return
     */
    @Override
    ...
    private void formatFacetsSection(final Composite parent) {
        ...
        upFacetAction = new UpFacetAction(mainWindow, ExternalizedStringProperties.create(
                "OtmW.66", "OtmW.67"));
        addAsNodeWithAction(upFacetAction);
        downFacetAction = new DownFacetAction(mainWindow, ExternalizedStringProperties.create(
                "OtmW.68", "OtmW.69"));
        ...
        IContributionItem addAction = RCPUtils.createCommandContributionItem(getSite(),
        ...
        final GridData extensionGD = new GridData(GridData.FILL_HORIZONTAL
        ...
        extendableAction = new ExtendableAction(mainWindow, ExternalizedStringProperties.create(
                "OtmW.74", "OtmW.75"), null);
        @SuppressWarnings("unused")
        ButtonWithAction extendableCheck = new ButtonWithAction(extensionCheckComposite, SWT.CHECK,
                extendableAction);
        addAsNodeWithAction(extendableAction);

        // Field and button for the base type
        extendsField = mc.getFields().formatTextField(extensionComposite,
                OtmTextFields.extendsName, 1);
        extendsField
                .setLayoutData(new GridData(GridData.FILL_HORIZONTAL | GridData.GRAB_HORIZONTAL));
        extendsField.setEditable(false);
        extendsSelector = mc.getFields().formatButton(extensionComposite,
                OtmWidgets.extendsSelector, OtmActions.extendsSelector(), null);
        extendsAction = new ExtendsAction(mainWindow, ExternalizedStringProperties.create(
                "OtmW.350", "OtmW.351"), extendsField, extendsSelector);

        extendsClearButton = mc.getFields().formatButton(extensionComposite,
                OtmWidgets.clearExtends, OtmActions.clearExtends(), null);
        extendsClearButton.setImage(Images.getImageRegistry().get("delete"));
        clearExtendsAction = new ClearExtendsAction(mainWindow,
                ExternalizedStringProperties.create("OtmW.352", "OtmW.353"), extendsField,
                extendsClearButton);
        ...
        table = toolkit.createTable(parent, SWT.MULTI | SWT.BORDER | SWT.FULL_SELECTION | SWT.CHECK
    ...
    private void postNode(Node target) {
        ...
        try {
            ...
            if (node instanceof FacetNode) {
                boolean edit = node.isEditable()
                        && !NodeUtils.checker(node).isInheritedFacet().get();
                mc.getFields().postField(nameField, node.getLabel(), edit);
            } else {
                mc.getFields().postField(nameField, node.getName(), node.isEditable());
            }
            if (node.isFacet() && !node.isOperation() && !node.isCustomFacet()
                    && !node.isQueryFacet())
     ...
     * (non-Javadoc)
     * 
     * @see com.sabre.schemas.views.OtmAbstractView#restorePreviousNode()
     */
    @Override
    public void restorePreviousNode() {
        ...
        LOGGER.debug("Restored previous node: " + prevNode);
    }
